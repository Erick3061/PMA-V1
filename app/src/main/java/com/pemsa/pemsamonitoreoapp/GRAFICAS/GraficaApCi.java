...
public class GraficaApCi {
    ...
    public ConstraintLayout Grafica(int tipo, ConstraintLayout graficas,JSONObject porcentajes,String total){
        ...
        switch (tipo){
            case 1:
                ...
                AperturaCierre.setPieChart(pieChart);
            ...
            case 3:
                ...
                AperturaCierre.setBarChart(barChart);
    ...
    private void loadPieChartDataApCi() {
        float aperturas,cierres;
        try {
            aperturas = Float.parseFloat(this.porcentajes.getString("aperturas"));
            cierres = Float.parseFloat(this.porcentajes.getString("cierres"));

            dataPie.add(new PieEntry(Math.round((aperturas*100)/Integer.parseInt(this.total)), "aperturas: "+Math.round((aperturas*100)/Integer.parseInt(this.total))+"%"));
            dataPie.add(new PieEntry(Math.round((cierres*100)/Integer.parseInt(this.total)), "cierres: "+Math.round((cierres*100)/Integer.parseInt(this.total))+"%"));
            ...
            dataSet.setColors(
                    ...
                    Color.parseColor("#"+String.valueOf(Integer.toHexString(activity.getColor(R.color.rojo)).substring(2))));
    ...
    private void loadBarChartDataApCi() {
        ...
        try {
            ...
            dataBars.add(new BarEntry(1,Math.round((aperturas*100)/Integer.parseInt(this.total))));
            dataBars.add(new BarEntry(2,Math.round((cierres*100)/Integer.parseInt(this.total))));

            String[] labels = new String[]{"","aperturas: "+Math.round((aperturas*100)/Integer.parseInt(this.total))+"%",
                    "cierres: "+Math.round((cierres*100)/Integer.parseInt(this.total))+"%"
            };
