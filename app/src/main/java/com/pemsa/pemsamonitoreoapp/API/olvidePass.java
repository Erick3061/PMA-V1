...
import com.pemsa.pemsamonitoreoapp.API.models.OlvidePass;
...
import com.pemsa.pemsamonitoreoapp.olvide_contra;
...
public class olvidePass extends AsyncTask<String, Void, String> {
    ...
    public static String ObtenerSesion(String cadena, String email) {
        String result = "";
        Parametros parametros = new Parametros();
        Retrofit retrofit = parametros.Connection("Sin token");
        OlvidePass olvidePass = new OlvidePass();
        JsonObject correo;
        correo= olvidePass.OlvidePass(email);
        InterfacesApi api = retrofit.create(InterfacesApi.class);
        Response<JsonElement> response2 = null;
        try {
            Response<JsonElement> response =api.olvidePass(correo).execute();
            response2=response;
        } catch (IOException e) {
            return e.getMessage();
        }
